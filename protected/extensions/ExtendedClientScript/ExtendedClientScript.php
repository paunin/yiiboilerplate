...
class ExtendedClientScript extends CClientScript
{
    ...
    public $excludeCssFiles = null;
    public $excludeJsFiles = null;
	/**
		...
		if ($this->combineJs)
		{
			if (isset($this->scriptFiles[$pos]) && count($this->scriptFiles[$pos]) !==  0)
			{
				...
				foreach ($jsFiles as &$fileName) {
                    ...
                    ((!empty($this->excludeJsFiles) && (in_array($file_arr['basename'], $this->excludeJsFiles) || $this->excludeJsFiles[0]=='*')) || $this->isRemoteFile($fileName)) AND $fileName = false;
                }
